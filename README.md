<!-- markdownlint-disable MD033 -->

<h1 align="center">📦 Icons Library</h1>

<p align="center">
  <strong>Easily browse and copy icons from popular free icon packs</strong>
</p>

<p align="center">
  <a href="https://open-vsx.org/extension/crsx/icons-library"><img src="https://img.shields.io/open-vsx/v/crsx/icons-library?logo=open-vsx&logoColor=white" alt="Open VSX Version"></a>
  <a href="https://open-vsx.org/extension/crsx/icons-library"><img src="https://img.shields.io/open-vsx/dt/crsx/icons-library" alt="Open VSX Downloads"></a>
  <a href="https://github.com/crsmilitaru97/icons-library"><img src="https://img.shields.io/badge/GitHub-Repository-181717?logo=github&logoColor=white" alt="GitHub"></a>
  <a href="https://github.com/crsmilitaru97/icons-library/stargazers"><img src="https://img.shields.io/github/stars/crsmilitaru97/icons-library" alt="GitHub Stars"></a>
  <a href="https://github.com/crsmilitaru97/icons-library/blob/main/LICENSE"><img src="https://img.shields.io/github/license/crsmilitaru97/icons-library" alt="License"></a>
  <a href="https://www.paypal.com/donate?hosted_button_id=MZQS5CZ68NGEW"><img src="https://img.shields.io/badge/Donate-PayPal-00457C?logo=paypal&logoColor=white" alt="Donate"></a>
</p>

---

<p align="center">
  <img src="https://raw.githubusercontent.com/crsmilitaru97/icons-library/main/assets/example.png" alt="Icons Library Preview" style="max-width: 100%; max-height: 500px; width: auto; height: auto; border-radius: 12px; border: 1px solid #30363d; box-shadow: 0 4px 10px rgba(0,0,0,0.25);">
</p>

## ✨ Features

- **Browse Icons**: View icons from multiple popular icon packs in one place.

- **One-Click Copy**: Click an icon to copy its class name to your clipboard.

- **Search**: Quickly filter icons by name or related keywords.

- **Configurable**: Choose which icon packs to display in the library.

## 📚 Included Packs

- [VS Code Codicons](https://microsoft.github.io/vscode-codicons/dist/codicon.html)
- [Font Awesome Free](https://fontawesome.com/)
- [PrimeIcons](https://primefaces.org/primeicons/)
- [Bootstrap Icons](https://icons.getbootstrap.com/)
- [Material Symbols](https://fonts.google.com/icons)
- [Heroicons](https://heroicons.com/)
- [Lucide](https://lucide.dev/)
- [Feather](https://feathericons.com/)
- [Tabler](https://tabler.io/)
- [Remixicon](https://remixicon.com/)
- [Lineicons](https://lineicons.com/)
- [Simple Icons](https://simpleicons.org/)
- [Eva Icons](https://akveo.github.io/eva-icons/)
- [Boxicons](https://boxicons.com/)
- [Iconoir](https://iconoir.com/)
- [Phosphor Icons](https://phosphoricons.com/)

> **Note:** This extension requires an active internet connection to load icon assets from CDNs.

## 📝 Configuration

- `iconsLibrary.enabledPacks`: Control which icon packs are displayed.
  - Default: All supported packs enabled.
  - Example: `["codicons", "fontawesome", "lucide"]`

---

<p align="center">
  <strong>💖 Support the Development</strong><br><br>
  If you find this extension useful, consider buying me a coffee!<br><br>
  <a href="https://www.paypal.com/donate?hosted_button_id=MZQS5CZ68NGEW">
    <img src="https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif" alt="Donate with PayPal" />
  </a>
</p>

---

<p align="center">
  <strong>🙏 Thank you for using Icons Library!</strong>
</p>
