...
export const CONSTANTS = {
  ...
  URLS: (config.URLS || {}) as Record<string, string>,
...
export const extractJson = <T = unknown>(text: string): T | null => {
  ...
  const attempt1 = safeJsonParse<T>(cleanText, null as any);
  if (attempt1) {return attempt1;}

  const match = cleanText.match(/```json([\s\S]*?)```/);
  if (match) {
    const attempt2 = safeJsonParse<T>(match[1], null as any);
  ...
  if (start !== -1 && end !== -1) {
    return safeJsonParse<T>(cleanText.substring(start, end + 1), null as any);
  }

  return null;
};

export const safeJsonParse = <T = unknown>(text: string, fallback: T): T => {
  try {
...
export const sanitizeTags = (tags: unknown[], iconName: string, maxTags = 100): string[] => {
...
export const log = {
  info: (msg: string) => process.stdout.write(`${CLR.cyan}ℹ${CLR.reset} ${msg}\n`),
  success: (msg: string) => process.stdout.write(`${CLR.green}✓${CLR.reset} ${msg}\n`),
  warn: (msg: string) => process.stdout.write(`${CLR.yellow}⚠${CLR.reset} ${msg}\n`),
  error: (msg: string) => process.stdout.write(`${CLR.red}✗${CLR.reset} ${msg}\n`),
  ...
  pack: (name: string) => process.stdout.write(`\n${CLR.bold}${CLR.magenta}▶ ${name}${CLR.reset}\n`),
};
...
export const checkOllamaModel = async (): Promise<boolean> => {
